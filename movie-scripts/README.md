...
Localization smoothing changes only the cut position. All displayed vorticity fields retain their original spatial detail. `frame.geostrophicPeak` records the selected grid index, coordinates, depth, unfiltered geostrophic vorticity at the tracked column minimum, and filtered localization value. `frame.trackingStatus` reports the selected association status. The tracked location need not be the instantaneous minimum of the unfiltered field.
...
The renderer verifies that `qg + qw` reconstructs `wvt.zeta_z/wvt.f` to numerical precision at every frame. The wave field includes all internal gravity wave modes, including waves generated by the evolving flow. Horizontally uniform inertial motion has zero vertical vorticity.
