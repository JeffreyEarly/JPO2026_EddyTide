...
See [movie-scripts](movie-scripts/README.md) for the 3D cutaway movie of vertical vorticity. After adding `movie-scripts` to the MATLAB path, run `MakeEddyTideCutawayMovie(frameRate=30)` for the full unforced simulation or `MakeEddyTideCutawayFrame(day=400)` for one still. Defaults include separate wave and geostrophic palettes, soft lighting, Optima typography, continuous anticyclonic tracking, and fixed color ranges of ±0.08 and ±0.12, respectively. The default `outputStride=2` selects 1,201 snapshots at 12-hour intervals, producing a 40.033-second 1920 × 1080 H.264 movie at 30 fps and Quality 95. For final movies, use `outputFormat="png",resolutionScale=2,outputFolder="movie-frames/eddy-tide-sequence-4k"` to render numbered 3840 × 2160 PNGs, then encode in Image2Movie with HEVC, High quality, and 30 fps. Keep the direct MATLAB MP4 path for previews. Current PNGs contain 8 bits per color channel; HEVC Main10 cannot recover precision absent from those source frames. The 12-hour cadence emphasizes eddy evolution and can alias tidal motion. See the [production workflow](movie-scripts/README.md#production-workflow-4k-png-frames-and-image2movie) for commands, metadata, and sampling guidance.
